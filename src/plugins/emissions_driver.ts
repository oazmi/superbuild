...
import { array_isEmpty, isArray, isNull, type MaybePromiseLike, promise_all, promiseOutside, textDecoder } from "../deps.js"
...
import type { longBuildPlugin } from "./long_build.js"
	...
	return (build: EsbuildPluginBuild | SuperPluginBuild) => {
		const
			base_plugin_build = INNER_PLUGIN_BUILD in build
				...
				: build,
			onEmitHandlers = buildCtx.onEmitHandlers,
			onEndHandlers = buildCtx.onEndHandlers
		...
		base_plugin_build.onEnd(async (result) => {
			const
				metafile = buildCtx.createMetafile(result),
...
interface EmissionDriverContext {
	/** the {@link SuperBuildContext} associated with the current build.
	 * it'll mostly get used for retrieving the long-build controller, and for utilizing the path resolver method.
	*/
	buildCtx: SuperBuildContext
...
const findLongBuildFile = (ctx: EmissionDriverContext): OutputFileEntity | undefined => {
	...
	const
		longbuild_plugin_namespace = buildCtx.longBuildController.pluginNamespace,
...
const incorporateLongBuildImportedEntities = async (
	...
	const
		...
		import_entities = await buildCtx.longBuildController.parseLongBuildFileContent(longbuild_contents)
		...
		const imported_entity_nodes: Array<ImportedEntityNode> = entities_to_import.map((import_entity) => {
			...
			const
				...
				entity: ImportedEntityNode["entity"] = external
					? { externalPath: path }
					: metafile.getFile(buildCtx.resolvePath(longbuild_path, path))!
